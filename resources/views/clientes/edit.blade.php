...
<div class="mb-6">
    ...
    <h1 class="mt-3 text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">Editar Cliente</h1>
    <p class="mt-1 text-sm text-slate-500">Actualiza los datos de contacto y configuración de {{ $cliente->nombre }}.</p>
</div>

<form action="{{ route('clientes.update', $cliente) }}" method="POST" class="max-w-3xl rounded-2xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-sm">
    @csrf
    @method('PUT')
